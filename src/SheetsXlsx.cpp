...
namespace {
const QString ns = QStringLiteral("http://schemas.openxmlformats.org/spreadsheetml/2006/main");
const QString relNs = QStringLiteral("http://schemas.openxmlformats.org/officeDocument/2006/relationships");
QString colName(int column) { QString s; for (++column; column; column = (column - 1) / 26) s.prepend(QChar('A' + (column - 1) % 26)); return s; }
int colIndex(const QString &s) { int n=0; for (QChar c:s) { if (!c.isLetter()) return -1; n=n*26+c.toUpper().unicode()-'A'+1; } return n-1; }
void start(QXmlStreamWriter &w,const QString &name) { w.writeStartElement(name); }
QByteArray makeXml(const std::function<void(QXmlStreamWriter&)> &body) { QByteArray data; QXmlStreamWriter w(&data); w.setAutoFormatting(false); w.writeStartDocument(); body(w); w.writeEndDocument(); return data; }
QString relationPart(const QString &part) { const int slash=part.lastIndexOf('/'); return part.left(slash+1)+"_rels/"+part.mid(slash+1)+".rels"; }
}

bool SheetsDocument::exportXlsx(const QUrl &url) {
    ...
    parts["[Content_Types].xml"]=makeXml([&](QXmlStreamWriter &w){start(w,"Types");w.writeDefaultNamespace("http://schemas.openxmlformats.org/package/2006/content-types");
        ...
        auto over=[&](QString part,QString type){start(w,"Override");w.writeAttribute("PartName",part);w.writeAttribute("ContentType",type);w.writeEndElement();};
        ...
        for(int i=0;i<sheets.size();++i)over("/xl/worksheets/sheet"+QString::number(i+1)+".xml","application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml");
        over("/xl/styles.xml","application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml");w.writeEndElement();});
...
bool SheetsDocument::importXlsx(const QUrl &url) {
    const QString path=url.isLocalFile()?url.toLocalFile():QString();int error=0;zip_t *archive=path.isEmpty()?nullptr:zip_open(QFile::encodeName(path).constData(),ZIP_RDONLY,&error);
    if(!archive){setError(QStringLiteral("Could not open the XLSX file."));return false;}
    auto fail=[&](){zip_close(archive);setError(QStringLiteral("This XLSX file has an unsupported or invalid worksheet."));return false;};
    QString workbook="xl/workbook.xml";auto root=OfficeZip::relationships(OfficeZip::read(archive,"_rels/.rels"));if(!root.isEmpty())workbook=OfficeZip::resolve({},root.constBegin().value());
